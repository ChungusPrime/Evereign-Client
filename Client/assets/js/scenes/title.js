...
export default class Title extends Phaser.Scene {
  ...
  preload () {
    ...
    this.load.html('CharacterCreationForm', 'assets/html/CharacterCreation.html');
  }
  ...
  create () {
    ...
    this.loginPanel.add([ this.loginForm, this.titleText, this.loginButton, this.signUpButton, this.testButton, this.test2Button ]);
  ...
  CreateNewCharacter() {
    ...
    const charName = this.createCharacterForm.getChildByName('name').value;
    if ( charName == "" ) return;

    const charClass = this.createCharacterForm.getChildByName('class').value;
    if ( charClass == "" ) return;

    const charFaction = this.createCharacterForm.getChildByName('faction').value;
    if ( charFaction == "" ) return;

    const charRace = this.createCharacterForm.getChildByName('race').value;
    if ( charRace == "" ) return;
