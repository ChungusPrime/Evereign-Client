...
export default class CharacterList extends Phaser.GameObjects.Container {
  ...
  constructor ( scene: Menu, x: number, y: number ) {
    ...
    // Scrollable panel containing list of characters
    this.Panel = new Panel(scene, "Characters", 4, 4, this.scene.scale.width * 0.3, this.scene.scale.height * 0.8 );

    this.CreateCharacterButton = new ModularButton(
      this.scene,
      this.Panel.ContentBackground.getBottomLeft().x + 4,
      this.Panel.ContentBackground.getBottomLeft().y + 15,
      this.Panel.ContentBackground.width,
      60,
      "Grey",
      "Create Character",
      this.ShowCreateCharacter.bind(this),
      0
    );
    ...
    this.Panel.Camera.ignore([
      this.CreateCharacterButton,
      this.BackToServerListButton,
      this.PlayButton,
      this.LogoutButton,
      this.scene.Background,
      this.scene.CharacterCreationPanel.ClassSelectionScreen,
      this.scene.CharacterCreationPanel.RaceSelectionScreen,
      this.scene.CharacterCreationPanel.AttributeAssignmentScreen,
      this.scene.CharacterCreationPanel.FactionSelectionScreen
    ]);
  ...
  ShowCreateCharacter () {
    ...
    this.scene.CharacterCreationPanel.setVisible(true);
  }
